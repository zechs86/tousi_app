...
# --- 優待の価値(利回り計算用・任意) ---
# 「コード: 1年あたりの優待価値(円)」。設定すると 🗓️予定 に優待利回り＆総合利回り(配当+優待)を表示。
# ※イオン(8267)のオーナーズカードは“買物○%キャッシュバック”で固定額でないため、
#   ご自身のイオンでの年間利用額×3%(感謝デー等)を目安に金額を入れてください(空なら配当利回りのみ表示)。
#   例) 年20万円利用×3% ≒ 6000円相当 → "8267.T": 6000
YUTAI_VALUE_YEN = {
    # "8267.T": 6000,
}

# --- 適時開示(TDnet)アラート(無料・通知) ---
# お気に入り＋保有銘柄に“注目度の高い公式開示”(決算/業績修正/配当/優待変更等)が出たら通知。
# 重複しないよう既出idは記録。Falseでオフ。
TDNET_ALERT_ENABLED = True
...
# --- 押し目買いアラート(お気に入り銘柄・無料) ---
# お気に入り銘柄が「上昇トレンド中(SMA25>SMA75)にRSIがこの値以下」に入ったら、
# 押し目買いゾーンとして通知する。watch.pyの改良③ロジックと同じ。Falseでオフ。
DIP_ALERT_ENABLED = True
DIP_RSI = 40            # この値以下を押し目とみなす(小さいほど厳しめ)
...
AI_NEWS_SUMMARY_ENABLED = False  # 📰ニュースのAI要約ボタン(押した時だけ約¥1〜2課金)。Trueで表示。
