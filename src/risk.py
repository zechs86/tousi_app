...
def suggest_shares(price, stop_pct, budget, is_jp=True):
    """上限損失(budget円)と損切り幅(stop_pct%)から「買ってよい株数」を逆算。
    株数 = budget ÷ (price × stop_pct%)。単元(日本株100株)に切り捨て。
    戻り値: dict(raw, unit, unit_shares, loss_per_share, need, max_loss, stop_price)。"""
    price = float(price or 0)
    loss_per_share = price * stop_pct / 100 if (price > 0 and stop_pct > 0) else 0.0
    raw = int(budget // loss_per_share) if loss_per_share > 0 else 0
    unit = 100 if is_jp else 1
    unit_sh = (raw // unit) * unit
    return {
        "raw": raw, "unit": unit, "unit_shares": unit_sh,
        "loss_per_share": loss_per_share,
        "need": unit_sh * price, "max_loss": unit_sh * loss_per_share,
        "stop_price": price * (1 - stop_pct / 100) if price else 0.0,
    }
